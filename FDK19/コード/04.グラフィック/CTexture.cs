...
namespace FDK
{
    public class CTexture : IDisposable
    {
        ...
        public void t2D拡大率考慮上中心基準描画(Device device, int x, int y)
        {
            this.t2D描画(device, x - (this.szテクスチャサイズ.Width / 2), y, 1f, this.rc全画像);
        }
        public void t2D拡大率考慮上中心基準描画(Device device, int x, int y, Rectangle rc画像内の描画領域)
        {
            this.t2D描画(device, x - (rc画像内の描画領域.Width / 2), y, 1f, rc画像内の描画領域);
        }
        public void t2D拡大率考慮上中心基準描画(Device device, float x, float y)
        {
            this.t2D拡大率考慮上中心基準描画(device, (int)x, (int)y);
        }
        ...
        public void t2D拡大率考慮中央基準描画(Device device, int x, int y, Rectangle rc画像内の描画領域)
        {
            this.t2D描画(device, x - (rc画像内の描画領域.Width / 2.0f * this.vc拡大縮小倍率.X), y - (rc画像内の描画領域.Height / 2.0f * this.vc拡大縮小倍率.Y), 1f, rc画像内の描画領域);
        }
